...
class MnnvlMemory:  # type: ignore[no-redef]
    ...
    def zero_local_segment(self) -> None:
        """Clear this rank's local workspace segment in place."""
        MnnvlMemory.zero_local_mnnvl_segment(self.ptr)
    ...
    @staticmethod
    def zero_local_mnnvl_segment(ptr: int) -> None:
        record = MnnvlMemory.allocated_map[ptr]
        if not record.mapped:
            raise RuntimeError("Cannot zero a detached MNNVL allocation")
        comm = record.comm
        rank_ptr = (
            record.start_address
            + comm.Get_rank() * record.rank_stride
            + record.address_offset
        )
        checkCudaErrors(cuda.cuMemsetD8(rank_ptr, 0, record.aligned_size))
